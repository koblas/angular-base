...
(function () {
    function b(a, d) {
        ...
        if (!this.config.disableStats) this.timelineSender = new b.TimelineSender(this.timeline, {
            host: this.config.statsHost,
            path: "/timeline"
        });
        this.connection = new b.ConnectionManager(this.key, b.Util.extend({
                getStrategy: function (a) {
                    return b.StrategyBuilder.build(b.getDefaultStrategy(c.config), b.Util.extend({}, c.config, a))
                },
    ...
    c.disconnect = function () {
        ...
        if (this.timelineSenderTimer) this.timelineSenderTimer.ensureAborted(), this.timelineSenderTimer = null
    };
    c.bind = function (a,
        d) {
    ...
    c.subscribe = function (a) {
        ...
        this.connection.state === "connected" && a.subscribe();
        return a
    };
    c.unsubscribe = function (a) {
        a = this.channels.remove(a);
        this.connection.state === "connected" && a.unsubscribe()
    };
    ...
    c.isEncrypted = function () {
        return b.Util.getDocumentLocation().protocol === "https:" ? !0 : Boolean(this.config.encrypted)
    };
    this.Pusher = b
}).call(this);
(function () {
...
(function () {
    Pusher.Util = {
        ...
        stringify: function () {
            for (var b = ["Pusher"], c = 0; c < arguments.length; c++) typeof arguments[c] === "string" ? b.push(arguments[c]) : window.JSON === void 0 ? b.push(arguments[c].toString()) :
                b.push(JSON.stringify(arguments[c]));
            return b.join(" : ")
        },
...
(function () {
    ...
    d.load = function (b, d) {
        var c = this;
        this.loaded[b] ? d() : this.loading[b] && this.loading[b].length > 0 ? this.loading[b].push(d) : (this.loading[b] = [d], a(this.getPath(b), function () {
            c.loaded[b] = !0;
            if (c.loading[b]) {
...
(function () {
    ...
    b.DEBUG = 7;
    c.log = function (a, c) {
        if (this.options.level === void 0 || a <= this.options.level) this.events.push(Pusher.Util.extend({}, c, {
            timestamp: Pusher.Util.now(),
            level: a !== b.INFO ? a : void 0
...
(function () {
    ...
    b.prototype.send = function (b, a) {
        if (!this.timeline.isEmpty()) {
            var d = this,
                h = "http" + (b ? "s" : "") + "://";
            d.timeline.send(function (a, b) {
                var c = {
                    data: Pusher.Util.filterObject(a, function (a) {
                        return a !== void 0
                    }),
                    url: h + (d.host || d.options.host) + d.options.path,
                    receiver: Pusher.JSONP
                };
                return Pusher.JSONPRequest.send(c, function (a, c) {
                    if (c && c.host) d.host = c.host;
                    b && b(a, c)
                })
            }, a)
        }
    ...
    h.connect =
        function (b, h) {
            var g = this.encrypted,
                i = a(g),
                j = [this.strategy];
            if (i && i.timestamp + this.ttl >= Pusher.Util.now()) {
                var k = this.transports[i.transport];
                k && (this.timeline.info({
                    cached: !0,
                    transport: i.transport
                }), j.push(new Pusher.SequentialStrategy([k], {
                    timeout: i.latency * 2,
                    failFast: !0
                })))
            }
            var m = Pusher.Util.now(),
                l = j.pop().connect(b, function p(a, i) {
                    if (a) d(g), j.length > 0 ? (m = Pusher.Util.now(), l = j.pop().connect(b, p)) : h(a);
                    else {
                        var k = i.transport.name,
                            q = Pusher.Util.now() - m,
...
(function () {
    ...
    c.connect = function (a, b) {
        var c = this,
            ...
            i = null,
            j = function (k, m) {
                m ? b(null, m) : (f += 1, c.loop && (f %= e.length), f < e.length ? (g && (g *= 2, c.timeoutLimit && (g = Math.min(g, c.timeoutLimit))), i = c.tryStrategy(e[f],
                    a, {
                        timeout: g,
                        failFast: c.failFast
                    }, j)) : b(!0))
...
(function () {
    ...
    a.connect = function (a, b) {
        ...
        var e = this,
            f = !1,
            g = this.transport.createConnection(this.name,
                this.priority, this.options.key, this.options),
            i = null,
            j = function () {
...
(function () {
    ...
    c.createConnection = function (a, b, c, e) {
        var f = this,
            e = Pusher.Util.extend({}, e, {
                activityTimeout: f.pingDelay
            }),
            g = f.transport.createConnection(a, b, c, e),
            i = null,
            j = function () {
                g.unbind("open", j);
                g.bind("closed", k);
                ...
                g.unbind("closed", k);
                if (a.code === 1002 || a.code === 1003) f.manager.reportDeath();
                else if (!a.wasClean && i &&
                    (a = Pusher.Util.now() - i, a < 2 * f.maxPingDelay)) f.manager.reportDeath(), f.pingDelay = Math.max(a / 2, f.minPingDelay)
            };
        g.bind("open", j);
        return g
    };
...
(function () {
    ...
    function a(a, b) {
        if (typeof a[0] === "string" && a[0].charAt(0) === ":") {
            var h = b[a[0].slice(1)];
            if (a.length > 1) {
                if (typeof h !== "function") throw "Calling non-function " + a[0];
                var e = [Pusher.Util.extend({}, b)].concat(Pusher.Util.map(a.slice(1), function (a) {
    ...
    var h = {
        ...
        sockjs: Pusher.SockJSTransport
            ...
            def_transport: function (a, b, c, d, f, l) {
                var n = h[c];
                if (!n) throw new Pusher.Errors.UnsupportedTransport(c);
                c = (!a.enabledTransports || Pusher.Util.arrayIndexOf(a.enabledTransports, b) !== -1) && (!a.disabledTransports || Pusher.Util.arrayIndexOf(a.disabledTransports, b) === -1) && (b !== "flash" || a.disableFlash !== !0) ? new Pusher.TransportStrategy(b, d, l ? l.getAssistant(n) :
...
(function () {
    Pusher.Protocol = {
        ...
        processHandshake: function (b) {
            ...
            if (b.event === "pusher:connection_established") {
                if (!b.data.activity_timeout) throw "No activity timeout specified in handshake";
                return {
                    action: "connected",
                    id: b.data.socket_id,
                    activityTimeout: b.data.activity_timeout * 1E3
                }
        ...
        var a = this,
            b = function (b) {
                ...
                a.transport.unbind("closed",
                    e);
                a.transport.unbind("error", c);
                a.transport.unbind("message", b);
                f && f.code && a.handleCloseEvent(f);
                a.transport = null;
                a.emit("closed")
            };
        a.transport.bind("message", b);
        a.transport.bind("error", c);
...
(function () {
    function b(a, b) {
        ...
        Pusher.Network.bind("online", function () {
            ...
            (c.state === "connecting" || c.state ===
                "unavailable") && c.retryIn(0)
        });
        Pusher.Network.bind("offline", function () {
            c.timeline.info({
                netinfo: "offline"
            });
            c.state === "connected" && c.sendActivityCheck()
        });
        this.updateStrategy()
    }
    var c = b.prototype;
    ...
    c.startConnecting = function () {
        var a = this,
            b = function (c, e) {
                c ? a.runner = a.strategy.connect(0, b) : e.action === "error" ? (a.emit("error", {
                    type: "HandshakeError",
                    error: e.error
                }), a.timeline.error({
                    handshakeError: e.error
                })) : (a.abortConnecting(), a.handshakeCallbacks[e.action](e))
            };
        a.runner =
            a.strategy.connect(0, b)
    };
    c.abortConnecting = function () {
        if (this.runner) this.runner.abort(), this.runner = null
    };
    ...
    c.retryIn = function (a) {
        ...
        a >
            0 && b.emit("connecting_in", Math.round(a / 1E3));
    ...
    c.clearRetryTimer = function () {
        if (this.retryTimer) this.retryTimer.ensureAborted(), this.retryTimer = null
    };
    ...
    c.resetActivityCheck = function () {
        var a = this;
        a.stopActivityCheck();
        if (!a.connection.supportsPing()) a.activityTimer = new Pusher.Timer(a.activityTimeout, function () {
            a.sendActivityCheck()
        })
    };
    ...
    c.shouldRetry = function () {
        return this.state ===
            "connecting" || this.state === "connected"
    };
...
(function () {
    ...
    c.trigger = function (a, b) {
        if (a.indexOf("client-") !== 0) throw new Pusher.Errors.BadEventName("Event '" + a + "' does not start with 'client-'");
...
(function () {
    ...
    Pusher.authorizers = {
        ...
        jsonp: function (c, a) {
            ...
            e.src = this.options.authEndpoint + "?callback=" + encodeURIComponent("Pusher.auth_callbacks['" + d + "']") + this.composeQuery(c);
